...
namespace StarForce
{
    public class FGuiFormHelper : UIFormHelperBase
    {
        ...
        {
            GameObject uiFormInstance = (GameObject) Instantiate((Object) uiFormAsset);
            FGuiForm m_FGuiForm = uiFormInstance.GetComponent<FGuiForm>();
            if (uiFormInstance.GetComponent<UIPanel>() == null)
            {
                throw new GameFrameworkException("UIPanel is not valid");
            }
            GameEntry.FGUI.AddPackage(m_FGuiForm);
        ...
        {
            GameEntry.FGUI.RemoveRef(((GameObject) uiFormInstance).GetComponent<FGuiForm>());
